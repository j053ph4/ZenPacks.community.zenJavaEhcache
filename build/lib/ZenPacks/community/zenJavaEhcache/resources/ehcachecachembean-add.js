...
(function() {
            ...
            Ext.ComponentMgr.onAvailable('component-add-menu', function(config) {
                ...
                menuButton.menuItems.push({
                    xtype: 'menuitem',
                    text: _t('Add Ehcache Cache') + '...',
                    hidden: Zenoss.Security.doesNotHavePermission('Manage Device'),
                    handler: function() {
                        var win = new Zenoss.dialog.CloseDialog({
                            width: 300,
                            title: _t('Add Ehcache Cache'),
                            items: [{
                                xtype: 'form',
                                buttonAlign: 'left',
                                monitorValid: true,
                                ...
                                items:                         [
                            {
                                ...
                                buttons: [{
                                    xtype: 'DialogButton',
                                    id: 'zenJavaEhcache-submit',
                                    text: _t('Add'),
                                    formBind: true,
                                    handler: function(b) {
                                        var form = b.ownerCt.ownerCt.getForm();
                                        var opts = form.getFieldValues();
                                        Zenoss.remote.zenJavaEhcacheRouter.addEhcacheCacheMBeanRouter(opts,
                                        function(response) {
                                            if (response.success) {
                                                new Zenoss.dialog.SimpleMessageDialog({
                                                    title: _t('Ehcache Cache Added'),
